...
exports.onCreateNode = ({ node, getNode, actions }) => {
  ...
  if (node.internal.type === 'MarkdownRemark') {
    ...
    const pathElements = getNode(node.parent).relativeDirectory.match(
      /(.+\/)?(\d{8})?-?(.+)/,
    );
...
exports.createPages = ({ graphql, actions }) => {
  ...
  `).then((result) => {
    ...
    result.data.allMarkdownRemark.edges.forEach(({ node }) => {
      // Start using dynamic template switching when at least one file uses the frontmatter
      // template field.
      ...
      createPage({
        ...
        // component: component,
        component: path.resolve('src/components/Article/Article.jsx'),
        context: {
